...
:class: important
This page is reading only — there is no code to run. (If you do want to try something out as you read, the usual workflow applies: work in `.py` scripts in VS Code — see [Setting up an accessible workflow](../computing_env/accessible_setup.md) and [Working with Python scripts](../computing_env/working_with_scripts.md) — run a script in the Git Bash terminal with `python yourfile.py`, and press `Ctrl+Up` to read the output.)
:::
...
:::{admonition} Accessibility of Missing Semester
:class: note
Each Missing Semester lecture is published as a full written page of lecture notes — ordinary web text with headings and code blocks, which reads well with a screen reader — alongside an optional lecture video. You do not need to watch the videos to get the material.
:::
...
- **Debugging and profiling** ([Debugging and Profiling](https://missing.csail.mit.edu/2026/debugging-profiling/)) — working in short scripts, you could poke at a problem directly: add a `print`, rerun the script, read the traceback in the terminal with `Ctrl+Up`. That easy visibility fades once your code grows into larger programs with many functions, where you can no longer see what is happening *inside* a function from the outside. A **debugger** lets you pause a running program and step through its variables one line at a time, and a **profiler** measures which lines actually take the time — so when a long analysis is slow, you fix the part that matters instead of guessing.
...
- **Testing and code quality** ([Code Quality](https://missing.csail.mit.edu/2026/code-quality/)) — as the scripts in this course grew, you mostly checked your work by hand — printing shapes, statistics, and spot values and reading them in the terminal (the "Check it" habit). Automated tests, linters, and continuous integration do that checking for you, every time the code changes — giving you confidence that your results are reproducible (the software-side complement to the reproducibility ideas from the *Data* module) and that your code keeps working as it changes. In fact, the "Check it" blocks throughout this course — `assert`-style facts like a shape, an `np.allclose`, a `line.get_color()` — are exactly the raw material of automated tests; a test suite is just those checks made permanent.
